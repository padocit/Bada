										...
										DWORD dwVertexNum, DWORD dwTriGroupCount)
{
	...
	if (FAILED(pResourceManager->CreateVertexBuffer(sizeof(BasicVertex), dwVertexNum,
		&m_VertexBufferView, &m_pVertexBuffer, (void*)pVertexList)))
	{
		__debugbreak();
		goto lb_return;
	}
	...
	const WCHAR* wchTexFileName)
{
	...
	if (FAILED(pResourceManager->CreateIndexBuffer(dwTriCount * 3, &IndexBufferView, &pIndexBuffer,
		(void*)pIndexList)))
	{
		__debugbreak();
		goto lb_return;
...
void CBasicMeshObject::Draw(DWORD dwThreadIndex, ID3D12GraphicsCommandList* pCommandList, const XMMATRIX* pMatWorld)
{
	...
	// 따라서 draw()할 때마다 CBV는 ConstantBuffer Pool로부터 할당받고, 렌더링용 descriptor table(shader visible)은 descriptor pool로부터 할당 받는다.
	...
	// Descriptor Table
	// per Object
	CD3DX12_CPU_DESCRIPTOR_HANDLE Dest(cpuDescriptorTable, BASIC_MESH_DESCRIPTOR_INDEX_PER_OBJ_CBV,
		srvDescriptorSize);
	pD3DDevice->CopyDescriptorsSimple(1, Dest, pCB->CBVHandle, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
	...
	{
		// Root-parameter 1
		pCommandList->SetGraphicsRootDescriptorTable(1, gpuDescriptorTableForTriGroup); // per Tri-group
