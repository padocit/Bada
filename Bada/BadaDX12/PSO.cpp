...
static map< size_t, ID3D12PipelineState* > s_GraphicsPSOHashMap;
static map< size_t, ID3D12PipelineState* > s_ComputePSOHashMap;

void PSO::DestroyAll(void)
{
    for (auto& iter : s_GraphicsPSOHashMap)
    {
    ...
    for (auto& iter : s_ComputePSOHashMap)
    {
        if (iter.second != nullptr)
        {
            iter.second->Release();
...
void GraphicsPSO::Finalize(ID3D12Device* device)
{
    ...
    if (firstCompile)
    {
        ...
        HRESULT hr;
        if (FAILED(hr = device->CreateGraphicsPipelineState(&m_PSODesc, IID_PPV_ARGS(&m_PSO))))
        {
            if (hr == E_OUTOFMEMORY)
            {
                OutputDebugStringA("Out of memory while creating graphics pipeline state object.\n");
            }
            else
            {
                OutputDebugStringA("Failed to create graphics pipeline state object.\n");
			}
			__debugbreak();
        }
...
void ComputePSO::Finalize(ID3D12Device* device)
{
    ...
    {
        HRESULT hr;
        if (FAILED(hr = device->CreateComputePipelineState(&m_PSODesc, IID_PPV_ARGS(&m_PSO))))
        {
            if (hr == E_OUTOFMEMORY)
            {
                OutputDebugStringA("Out of memory while creating compute pipeline state object.\n");
            }
            else
            {
                OutputDebugStringA("Failed to create compute pipeline state object.\n");
            }
            __debugbreak();
        }
