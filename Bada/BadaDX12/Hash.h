...
namespace Utility
{
    ...
    {
#if ENABLE_SSE_CRC32
		const uint64_t* Iter64 = (const uint64_t*) (uint32_t)(((size_t)Begin + 7) & ~7); // Align up to 8 bytes (From DX12 Sample)
		const uint64_t* const End64 = (const uint64_t* const) (uint32_t)((size_t)End & ~7); // Align down to 8 bytes
