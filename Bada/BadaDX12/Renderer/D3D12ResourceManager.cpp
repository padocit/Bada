...
HRESULT CD3D12ResourceManager::CreateVertexBuffer(UINT SizePerVertex, DWORD dwVertexNum, D3D12_VERTEX_BUFFER_VIEW* pOutVertexBufferView, ID3D12Resource** ppOutBuffer, void* pInitData)
{
	...
	ID3D12Resource* pVertexBuffer = nullptr; // GPU Memory
	ID3D12Resource* pUploadBuffer = nullptr; // CPU Memory
	UINT		VertexBufferSize = SizePerVertex * dwVertexNum;

	// create vertexbuffer for rendering
	hr = m_pD3DDevice->CreateCommittedResource(
		&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), // GPU Memory
	...
	// CommandList: CPU -> GPU 전송 또한 CommandList에 기록하는 방식으로 진행해야함.
	if (pInitData)
	{
		...
		hr = m_pD3DDevice->CreateCommittedResource(
			&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), // CPU Memory
			D3D12_HEAP_FLAG_NONE,
			&CD3DX12_RESOURCE_DESC::Buffer(VertexBufferSize),
			D3D12_RESOURCE_STATE_COMMON,
			nullptr,
...
BOOL CD3D12ResourceManager::CreateTexture(ID3D12Resource** ppOutResource, UINT Width, UINT Height, DXGI_FORMAT format, const BYTE* pInitImage)
{
	...
	if (FAILED(m_pD3DDevice->CreateCommittedResource(
		...
		D3D12_HEAP_FLAG_NONE,
		&textureDesc,
		D3D12_RESOURCE_STATE_ALL_SHADER_RESOURCE, // NON_PS + PS
		nullptr,
	...
	if (pInitImage)
	{
		...
		for (UINT y = 0; y < Height; y++)
		{
			...
			pSrc += (Width * 4);
			pDest += Footprint.Footprint.RowPitch; // Linear memory to Swizzle format
		}
		// Unmap
		pUploadBuffer->Unmap(0, nullptr);
...
{
	const DWORD MAX_SUB_RESOURCE_NUM = 32; // max mip level 32
