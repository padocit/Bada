...
BOOL CFontManager::WriteTextToBitmap(BYTE* pDestImage, UINT destWidth, UINT destHeight, UINT destPitch,
	int* piOutWidth, int* piOutHeight, FONT_HANDLE* pFontHandle, const WCHAR* wchString, DWORD dwLen)
{
	int iTextWidth = 0;
	int iTextHeight = 0;
...
BOOL CFontManager::CreateBitmapFromText(int* piOutWidth, int* piOutHeight, IDWriteTextFormat* pTextFormat, const WCHAR* wchString, DWORD dwLen)
{
	...
	if (pTextLayout)
	{
		...
		// We ignore D2DERR_RECREATE_TARGET here. This error indicates that the device is lost. 
		// It will be handled during the next call to Present.
