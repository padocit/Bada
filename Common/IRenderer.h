...
interface IRenderer : public IUnknown
{
	...
	virtual IMeshObject* __stdcall CreateSkyboxMeshObject() = 0;
	...
	virtual	void*	__stdcall CreateMagentaTexture(UINT texWidth, UINT texHeight) = 0;
	...
	virtual void*	__stdcall CreateTextureFromFile(const WCHAR* wchFileName, BOOL bIsCubeMap = FALSE) = 0;
	...
	virtual void	__stdcall RenderSkybox(IMeshObject* pMeshObj, const XMMATRIX* pMatWorld) = 0;
	...
	virtual void	__stdcall SetCameraRot(float fPitch, float fYaw, float fRoll) = 0;
	virtual void	__stdcall RotateCamera(float fPitch, float fYaw, float fRoll) = 0;
	virtual void	__stdcall GetCameraRot(float* pfOutPitch, float* pfOutYaw, float* pfOutRoll) = 0;
	virtual void	__stdcall MouseRotateCameraDelta(float deltaX, float deltaY) = 0;
